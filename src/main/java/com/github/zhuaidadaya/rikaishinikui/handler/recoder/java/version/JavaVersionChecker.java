...
public class JavaVersionChecker {
    public JavaVersionInformation check(String path) {
        try {
            if (path.equals("path.java")) path = "java";
            ...
            if (path.equals("java") || filter1 || filter2 || filter3) {
                ...
                while ((cache = br.readLine()) != null) {
                    ...
                    if (line == 2) {
                        javaId = cache.substring(cache.lastIndexOf("(") + 1, cache.lastIndexOf(")"));
                    }
                }

                if (line < 3) throw new Exception();

                String verId = builder.substring(builder.indexOf("\"") + 1, builder.lastIndexOf("\""));
                JavaVersionInformation information = new JavaVersionInformation(path, verId, builder.toString().contains("64-Bit"), javaId, type);
                if (path.equals("java")) {
                ...
                try {
                    String version = verId.substring(0, 3);
                    if (version.startsWith("1.")) {
                        information.setVersion(Integer.parseInt(version.substring(2)));
                    } else {
