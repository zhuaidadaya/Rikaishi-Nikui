...
public class MinecraftClassifierParser {
    ...
    public MinecraftClassifierParser(JSONObject json, String os) {
    ...
    public MinecraftArtifactParser getNative() {
        try {
            if(rules != null) {
                initRulesAllow();
                natives.getString(os.equals("macos") ? "osx" : os);
                if(rulesAllow.get(os)) {
                    return new MinecraftArtifactParser(classifiers.getJSONObject("natives-" + os));
                }
            } else {
                natives.getString(os.equals("macos") ? "osx" : os);
                return new MinecraftArtifactParser(classifiers.getJSONObject("natives-" + os));
